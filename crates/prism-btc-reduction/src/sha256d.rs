...
///
/// SHA256d is a non-structure-preserving avalanche hash: total over its input
/// domain, not invertible, preserves no algebraic structure. The matching
/// foundation morphism kind is `DigestProjectionMap` — `Total`, but
/// neither `Invertible` nor `PreservesStructure`. That kind is carried at
/// the type level as the `Sigma` parameter on `BlockCertificate`.
///
