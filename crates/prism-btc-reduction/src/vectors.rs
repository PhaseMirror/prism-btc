...
///
/// This function runs the ψ-reduction pipeline over `BlockHash`'s
/// `ConstrainedTypeShape` constraints (W8 per-byte residue constraint, 32 sites).
/// It is called at test time as the genesis regression anchor.
///
...
pub fn genesis_grounded() -> Grounded<BlockHash> {
    uor_ground! {
